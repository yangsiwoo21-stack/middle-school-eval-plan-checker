...
class AssessmentCheckerApp(tk.Tk):
    ...
    def _build_ui(self) -> None:
        ...
        ttk.Label(title_box, text="평가계획 점검 도우미 v0.3", style="Title.TLabel").pack(anchor="w")
        ttk.Label(
            title_box,
            text="HWPX 평가계획을 섹터별로 읽고 엑셀형 Pass/Fail 검토표와 오류 후보를 함께 생성합니다.",
            style="Subtitle.TLabel",
        ).pack(anchor="w", pady=(6, 0))
        ttk.Label(
            title_box,
            text="추천 흐름: 폴더/파일 선택 → 점검 실행 → 엑셀 검토표 생성 → Fail/확인 필요 항목 검토",
            style="Subtitle.TLabel",
        ...
        ttk.Button(controls, text="엑셀 검토표 생성", command=self._save_excel_review, style="Primary.TButton").pack(side=tk.LEFT)
        ...
        try:
            while True:
                ...
                if event == "review_done":
                    ...
                    messagebox.showinfo(APP_TITLE, f"엑셀 검토표 생성 완료\n{payload}")
                elif event == "error":
    ...
    def _save_excel_review(self) -> None:
        if not self.last_review_path:
            messagebox.showwarning(APP_TITLE, "먼저 폴더 점검 또는 파일 점검을 실행해 주세요.")
            return
        out_dir = Path(self.output_dir.get())
        out_dir.mkdir(parents=True, exist_ok=True)
