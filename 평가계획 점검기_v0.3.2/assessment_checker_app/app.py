...
APP_TITLE = "평가계획 점검 도우미 v0.3.2 배포용"
...
class AssessmentCheckerApp(tk.Tk):
    ...
    def _build_ui(self) -> None:
        ...
        stage_label = ttk.Label(root, text="단계별 점검 결과", style="Panel.TLabel")
        stage_label.pack(anchor="w", pady=(4, 4))

        stage_columns = ("grade", "subject", "stage", "status", "message")
        self.stage_tree = ttk.Treeview(root, columns=stage_columns, show="headings", height=7)
        stage_headers = {
            "grade": "학년",
            "subject": "과목",
            "stage": "점검 단계",
        ...
        self.stage_tree.pack(fill=tk.X)
        ...
        findings_label = ttk.Label(root, text="수정 권고 및 메모 대상", style="Panel.TLabel")
        findings_label.pack(anchor="w", pady=(10, 4))
    ...
    def _save_excel_review(self, silent: bool = False) -> None:
        ...
        output_path = out_dir / f"{stem}_v0.3.2_엑셀검토표.xlsx"
        if not silent:
