...
KORDOC_PARSE_SCRIPT = APP_DIR / "kordoc_parse.mjs"
...
def parse_hwpx_with_kordoc(path: Path) -> str:
    if not KORDOC_PARSE_SCRIPT.exists():
        return ""
    ...
    try:
        result = subprocess.run(
            [node_exe, str(KORDOC_PARSE_SCRIPT), str(path)],
            cwd=str(APP_DIR),
            capture_output=True,
            text=True,
            encoding="utf-8",
...
class Document:
    ...
    def from_path(cls, path: Path) -> "Document":
        if path.suffix.lower() == ".hwpx":
            ...
            text = parse_hwpx_with_kordoc(path) or extract_visible_text(section_xml)
