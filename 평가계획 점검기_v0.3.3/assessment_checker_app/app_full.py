...
APP_TITLE = "평가계획 점검 도우미 v0.3.3 전체검증용"
...
class AssessmentCheckerApp(tk.Tk):
    def __init__(self) -> None:
        ...
        self.input_folder_display = tk.StringVar(value="폴더를 선택하세요")
    ...
    def _build_ui(self) -> None:
        ...
        ttk.Label(
            title_box,
            text="파일/폴더 선택 → 점검 → 메모본·엑셀 검토표 자동 저장",
        ...
        ttk.Label(top, text="평가계획 폴더", style="Panel.TLabel").grid(row=1, column=0, sticky="w", pady=(6, 0))
        ttk.Entry(top, textvariable=self.input_folder_display, state="readonly").grid(row=1, column=1, sticky="ew", padx=8, pady=(6, 0))
        ttk.Button(top, text="폴더 선택", command=self._choose_folder, style="App.TButton").grid(row=1, column=2, pady=(6, 0))
        ...
        ttk.Button(controls, text="폴더 점검", command=self._run_folder_review, style="Primary.TButton").pack(side=tk.LEFT, padx=(6, 0))
    ...
    def _choose_folder(self) -> None:
        current = Path(self.input_folder.get())
        initial_dir = str(current) if current.exists() else str(self.workspace_dir)
        path = filedialog.askdirectory(initialdir=initial_dir, title="평가계획 폴더 선택")
        if path:
            folder = Path(path)
            self.input_folder.set(str(folder))
            self.input_folder_display.set(self._short_path(folder))
    ...
    def _run_folder_review(self) -> None:
        path = Path(self.input_folder.get())
        if not path.exists() or not path.is_dir():
            messagebox.showerror(APP_TITLE, "평가계획 폴더를 선택해 주세요.")
            return
        self._set_output_dir(path)
        self._start_review("folder", path)
    ...
    def _save_excel_review(self, silent: bool = False) -> None:
        ...
        output_path = out_dir / f"{stem}_v0.3.3_엑셀검토표.xlsx"
        if not silent:
